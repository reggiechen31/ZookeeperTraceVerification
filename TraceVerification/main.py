...
'''
读取zookeeper日志，并且过滤10222803标签日志，并输入队列
Parameters:
...
def fileReadZookeeperLog(zookerloglocation):
    ...
    for (num, value) in enumerate(file):
        ...
        if '10222803' in value and '10222803 start connection' not in value and '10222803 end connection' not in value:
...
def selectZookeeperState(result,stateconsistencyout):
    ...
    for index in range(len(result)):
        if '10222803 follower sync data' in result[index]:
            leaderelectionList2[0] = index
        elif '10222803 follower send epoch' in result[index]:
    ...
    for index_1 in range(len(leaderelectionList)):
        ...
        for offset in range(leaderelectionList[index_1][0], leaderelectionList[index_1][1]):
            ...
            if '10222803 received vote' in result[offset]:
            ...
            if '10222803 leave FLE instance' in result[offset]:
...
def selectBrodCastLog(result,broadcastout):
    ...
    broadcastlist2 = [0,0]
    for index in range(len(result)):
        if ('FollowerRequestProcessor' in result[index]) and ('10222803 Processing request' in result[index]) and (' type:create ' in result[index]):
            broadcastlist2[0] = index
        if ('LeaderRequestProcessor' in result[index]) and ('10222803 Leader Request process' in result[index]) and (' type:create ' in result[index]):
            broadcastlist2[0] = index

        if ('FinalRequestProcessor' in result[index]) and ('10222803 Processing request to DataTree' in result[index]) and ('10222803 leader received ack size from follower,ack' in result[index + 1]):
    ...
    for index_1 in range(len(broadcastlist)):
        ...
        for temp in range(broadcastlist[index_1][0],broadcastlist[index_1][1]):
            ...
            if ('PrepRequestProcessor' in result[temp]) and ('10222803 createRequest' in result[temp]):
            ...
            if ('Leader' in result[temp]) and ('10222803 leader launch proposal' in result[temp]):
                myId = result[temp][(result[temp].find("[myid:") + 6):(result[temp].find("[myid:") + 7)]
            ...
            if '10222803 received request from leader' in result[temp]:
                myId = result[temp][(result[temp].find("[myid:") + 6):(result[temp].find("[myid:") + 7)]
                ...
                broadcastoutfile.write("state:" + str(
                    state) + ",myId:" + myId + ",action:" + action + ",sessionid:" + sessionid + ",type:" + type + ",cxid:" + cxid + ",zxidHigh:" + str(
                    zxidHigh) + ",zxidLow:" + str(zxidLow) + ",txntype:" + txntype + ",reqpath:" + reqpath + "\n")
            if ('SendAckRequestProcessor' in result[temp]) and ('10222803 follower send ACK to leader' in result[temp]):
                myId = result[temp][(result[temp].find("[myid:") + 6):(result[temp].find("[myid:") + 7)]
            ...
            if ('LearnerHandler' in result[temp]) and ('10222803 leader received ACK from follower' in result[temp]):
                myId = result[temp][(result[temp].find("[myid:") + 6):(result[temp].find("[myid:") + 7)]
            ...
            if ('LearnerHandler' in result[temp]) and ('10222803 send commit msg' in result[temp]):
                myId = result[temp][(result[temp].find("[myid:") + 6):(result[temp].find("[myid:") + 7)]
            ...
            if ('CommitProcessor' in result[temp]) and ('10222803 Committing request' in result[temp]):
                myId = result[temp][(result[temp].find("[myid:") + 6):(result[temp].find("[myid:") + 7)]
            ...
            if ('FinalRequestProcessor' in result[temp]) and ('10222803 Processing request to DataTree' in result[temp]):
                myId = result[temp][(result[temp].find("[myid:") + 6):(result[temp].find("[myid:") + 7)]
...
def slectLeaderSelection(leaderelectionout,result):
    ...
    for index_1 in range(len(leaderelectionList)):
        myidlist = []
        for offset in range(leaderelectionList[index_1][0], leaderelectionList[index_1][1]):
            if '10222803 end vote:' in result[offset]:
        ...
        for offset2 in range(len(myidlist)):
            ...
            for offset3 in range(leaderelectionList[index_1][0], leaderelectionList[index_1][1]):
                ...
                if (int(myidlist[offset2].split('--')[0]) == int(myidTemp)) and ( '10222803 received vote' in result[offset3]):
...
def logparser(result,connectout,sycdataout):
    ...
    for index2 in range(len(syncdataList)):
        ...
        for j in range(syncdataList[index2][0],syncdataList[index2][1]+1):
            ...
            if '10222803 follower send epoch' in result[j]:
...
def selectsnaprequest(result,snapfilesync):
    ...
    myIddlist = []
    for index in range(len(result)):
        if '10222803 request snap file' in result[index]:
...
def runVerification():
    ...
    print("**************************************************************************************************************")
    print("*********************************connect verification start***************************************************")
    print("**************************************************************************************************************")
    os.system("/media/vdc/software/jdk-11.0.7/bin/java -jar tla2tools.jar -config connect.cfg -cleanup -deadlock connect.tla")
    print("**************************************************************************************************************")
    print("*********************************leadervote verification start**************************************************")
    print("**************************************************************************************************************")
    os.system("/media/vdc/software/jdk-11.0.7/bin/java -jar tla2tools.jar -config leadervote.cfg -cleanup -deadlock leaderevote.tla")
    print("**************************************************************************************************************")
    print("*********************************datasync verification start************************************************")
    print("**************************************************************************************************************")
    os.system("/media/vdc/software/jdk-11.0.7/bin/java -jar tla2tools.jar -config datasync.cfg -cleanup -deadlock datasync.tla")
    print("**************************************************************************************************************")
    print("*********************************zookeeper state consistency verification************************************************")
    print("**************************************************************************************************************")
    os.system("/media/vdc/software/jdk-11.0.7/bin/java -jar tla2tools.jar -config StatesConsistencyInspect.cfg -cleanup -deadlock stateconsistency.tla")
    print("**************************************************************************************************************")
    print("*********************************zookeeper snap consistency verification************************************************")
    print("**************************************************************************************************************")
    os.system("/media/vdc/software/jdk-11.0.7/bin/java -jar tla2tools.jar -config SnapFileSync.cfg -cleanup -deadlock SnapSync.tla")
    print("**************************************************************************************************************")
    print("*********************************zookeeper state brodcast verification************************************************")
    print("**************************************************************************************************************")
    os.system("/media/vdc/software/jdk-11.0.7/bin/java -jar tla2tools.jar -config Broadcast.cfg -cleanup -deadlock BS.tla")
